        ...
        fn: {
            init: function () {
                ...
                $("#all-delete").click(function () {
                    ...
                    serviceType.fn.deleteRow(checkBox, ids)
                })
            },
            dataTableInit: function () {
                serviceType.v.dTable = $dh.dataTable($('#dataTables'), {
                    "processing": true,
                    "serverSide": true,
                    "ordering": false,
                    "searching": false,
                    "ajax": {
                        "url": "service/serviceTypePage",
                        "type": "POST"
                    },
                    "columns": [
                        {"data": "","defaultContent": ""},
                    ...
                    "rowCallback": function (row, data) {
                        $('td', row).last().find(".delete").click(function () {
                            ...
                            serviceType.fn.deleteRow(checkbox, [data.id]);
                        })

                        $('td', row).last().find(".edit").click(function () {
                            update=true;
                            create=false;
                            serviceType.fn.edit(data.id);
                            $('#save').show();
                        })
            ...
            deleteRow: function (checkBox, ids) {
                if (ids.length > 0) {
                    $dh.optNotify(function () {
                        $dh.ajax("service/deleteServiceType", {ids: JSON.stringify(ids)}, function (result) {
                            if (result.status == "0") {
            ...
            edit: function (id) {
                $.post("service/findServiceArea",{id:id},function(result){
                ...
                serviceType.fn.showModal("修改用户");
                var items = serviceType.v.list;
