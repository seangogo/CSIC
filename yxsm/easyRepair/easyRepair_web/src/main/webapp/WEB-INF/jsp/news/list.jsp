                    ...
                    <table id="dataTables"
                        ...
                        <tr role="row">
                            ...
                            <th class="sorting" tabindex="0" aria-controls="dynamic-table"
                                rowspan="1" colspan="1"
                                aria-label="Domain: activate to sort column ascending">新闻标题
                            </th>
                            <th class="hidden-480 sorting" tabindex="0"
                                aria-controls="dynamic-table" rowspan="1" colspan="1"
                                aria-label="Clicks: activate to sort column ascending">新闻描述
                            </th>
                            <th class="sorting" tabindex="0" aria-controls="dynamic-table"
                                rowspan="1" colspan="1"
                                aria-label="Price: activate to sort column ascending">新闻图片
                            </th>
                            <th class="hidden-480 sorting" tabindex="0"
                                aria-controls="dynamic-table" rowspan="1" colspan="1"
                            ...
                            <th class="hidden-480 sorting" tabindex="0"
                                aria-controls="dynamic-table" rowspan="1" colspan="1"
                                aria-label="Clicks: activate to sort column ascending">修改时间
                            </th>
                            <%--<th class="hidden-480 sorting" tabindex="0"
                                aria-controls="dynamic-table" rowspan="1" colspan="1"
                                aria-label="Clicks: activate to sort column ascending">是否展示
                            </th>--%>
                            <th class="hidden-480 sorting" tabindex="0"
                                aria-controls="dynamic-table" rowspan="1" colspan="1"
                                aria-label="Clicks: activate to sort column ascending">来源
                            </th>
                            <th class="hidden-480 sorting" tabindex="0"
                                aria-controls="dynamic-table" rowspan="1" colspan="1"
                                aria-label="Clicks: activate to sort column ascending">新闻分类
                            </th>
                            <%--<th class="col-lg-2">
                                <div class="hidden-sm hidden-xs action-buttons" >
                                    <a   title="查看" id="look-up"><i class="ace-icon fa fa-search-plus bigger-130 grey"></i></a>
                                    <a  title="新增" &lt;%&ndash;id="create"&ndash;%&gt;><i class="ace-icon fa fa-pencil bigger-130 purple"></i></a>
                                    <a   title="批量删除" &lt;%&ndash;id="all-delete"&ndash;%&gt;><i class="ace-icon fa fa-trash-o bigger-130 red"></i></a>
                                </div>
                            </th>--%>
                            <th class="hidden-480 sorting" tabindex="0"
                                aria-controls="dynamic-table" rowspan="1" colspan="1"
...
<div id="modal-form" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="newsForm" method="post" role="form">
...
<script type="text/javascript">
    var news = {
        ...
        fn: {
            init: function () {
                news.fn.dataTableInit();
                news.fn.dropperInit();//图片插件初始化
                news.fn.editorInit();
                news.fn.validator();
                news.fn.selectInit("news/allNewsType", $('#newsTypeId'));

                $(".remove-img").click(function () {
                    news.fn.clearImageView();
                });
                $("#create").click(function () {
                    if (news.v.last_gritter) $.gritter.remove(news.v.last_gritter);
                    $('#newsContent').html("");
                    $('#newsDesc').val("");
                    news.fn.clearImageView();
                    news.fn.showModal("新建新闻");
                });
                $("#save").click(function () {
                    news.fn.save();
                });
                ...
                $("#batchDel").click(function () {
                    ...
                    news.fn.deleteRow(ids)
                });
                $("#ui-search-btn").on("click", function () {
                    news.fn.search();//查询
                });
                $dh.tableSearch(news.v.dTable);
            ...
            dataTableInit: function () {
                news.v.dTable =$("#dataTables").DataTable({
                    "ajax": { // ajax请求
                        "type": "post",
                        "url": "news/page",
                        "data": function (d) {
                    ...
                    "rowCallback": function (row, data) {
                        $('td', row).last().find(".look-up").click(function () {
                            news.fn.look_up(data);//查看信息
                        });
                        $('td', row).last().find(".delete").click(function () {
                            news.fn.deleteRow([data.id]);
                        });
                        $('td', row).last().find(".edit").click(function () {
                            if (news.v.last_gritter) $.gritter.remove(news.v.last_gritter);
                            news.fn.edit(data);//修改单条记录
                        });
                    },
                ...
                $dh.tableTools(news.v.dTable, buttonId);
            },
            search: function () {
            ...
            dropperInit: function () {
                $("#headImage  .dropped").dropper({
                    ...
                    label: "把文件拖拽到此处"
                }).on("fileComplete.dropper", news.fn.onFileComplete)
                        .on("fileError.dropper", news.fn.onFileError)
            },
            ...
            onFileComplete: function (e, file, response) {
                if (response != '' && response != null) {
                    news.fn.viewImage(response.url);
                } else {
            ...
            deleteRow: function (ids) {
                if (ids.length > 0) {
                    $dh.optNotify(function () {
                        $dh.ajax("news/delete", {ids: JSON.stringify(ids)}, function (result) {
                            if (result.status == "0") {
            ...
            look_up: function (data) {
                ...
                var html = "";
                for (var key in data) {
                    if (news.v.property[key] && typeof (news.v.property[key]) == "string") {
                        ...
                        html += news.v.property[key] + ":" + propertyVal + "<br/>";
                    } else {
                        for (var key2 in data[key]) {
                            if (news.v.property[key] && news.v.property[key][key2]) {
                                if (typeof (data[key][key2]) == 'string') {
                                    if (key2 == 'name') {
                                        html += news.v.property[key][key2] + ":" + data[key][key2] + "<br/>";
                                    }
                                }
                            }
                        }
                ...
                if (news.v.last_gritter) $.gritter.remove(news.v.last_gritter);
                news.v.last_gritter = $.gritter.add({
                    title: title,
                    text: html,
            ...
            edit: function (item) {
                news.fn.clearImageView();
                $.post("news/findNewsType",{id:item.id},function(result){
                            news.fn.selectChoose($('#newsTypeId option'), result);
                        }, "json"
                );
                news.fn.showModal("修改新闻");
                for (var key in item) {
                    var element = $("#newsForm :input[name=" + key + "]")
                    if (element.length > 0) {
                        element.val(item[key]);
                    }
                    if (key == 'newsImg') {
                        if (item[key] != "") {
                            news.fn.viewImage(item[key]);
                        }
                    }
                    if(key=='isShow') {
                        if (item[key]) {
            ...
            save: function () {
                ...
                $("#newsForm").ajaxSubmit({
                    url: $('#id').val() != "" ? 'news/update':'news/create',
                    dataType: "json",
                    data: {newsContent: $('#newsContent').html()},
                    success: function (result) {
                        news.fn.responseComplete(result, true);
            ...
            responseComplete: function (result, action) {
                if (result.status == "0") {
                    if (action) {
                        news.v.dTable.ajax.reload(null, false);
                    } else {
                        news.v.dTable.ajax.reload();
                    }
