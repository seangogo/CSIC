...
(function (RongWebIMWidget) {
    ...
    var WebIMWidget = (function () {
        ...
        WebIMWidget.prototype.init = function (config) {
            ...
            _this.RongIMSDKServer.setOnReceiveMessageListener({//   接收到消息
...
//                  _this.$log.debug(data);
                  a=data;
                    ...
                    if (!document.hidden && _this.display
                        ...
                        _this.RongIMSDKServer.sendReadReceiptMessage(_this.conversationServer.current.targetId, _this.conversationServer.current.targetType);//接收消息
                    }
                    _this.conversationListServer.updateConversations().then(function () { });
                	 _this.RongIMSDKServer.getTotalUnreadCount();//显示未读消息条数
                }
            });
                ...
                if (conversationServer.current && conversationServer.current.targetId && WebIMWidget.display) {
                    RongIMSDKServer.getConversation(conversationServer.current.targetType, conversationServer.current.targetId).then(function (conv) {
                        if (conv && conv.unreadMessageCount > 0) {
                         ...
                         RongIMSDKServer.getTotalUnreadCount();//获取未读消息
                            conversationListServer.updateConversations().then(function () { });
                        }
                    });
                }
...
//                     $(".unreadmesg").each(function(){
//                     		console.log($(this).html());
//                     		if($(this).html()==data.latestMessage.content.content){
...
//                  if(num>0){
//                  	 $("#textcontent").html(
//                  	 	$("#textcontent").html()+"<div><span>用户:</span><span class='user'>"+a.content.user.name+"</span><span>未读消息：</span><span class='unreadmesg'>"+a.content.content+"</span></div>");
//                  }
                },
                onError: function () {
//                  defer.reject();
